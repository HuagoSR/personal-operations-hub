...
26 项：可靠性 Test 1–10（重复摄入/双审批/crash 恢复/重复派发/Worker 失败/WAITING_FOR_USER 重启恢复/审批过期/Grant 吊销/Result 不可变/Worker 不能完成 Task）+ CRASH/TIMEOUT/权限路径 + 状态机与 ingest 边界 + API happy path 与负面用例。
