...
sed -i 's/deepseek-chat/deepseek-v4-flash/g' $H/.opencode/opencode.json
echo "=== config model lines ==="
grep -n 'deepseek' $H/.opencode/opencode.json | head -5
echo "=== CLI run v4-flash ==="
timeout -k 5 60 bwrap --unshare-user --unshare-ipc --unshare-pid --unshare-uts --unshare-cgroup \
  ...
  --die-with-parent -- /opt/opencode-bin/opencode run --print-logs "Reply with exactly: PONG" 2>&1 | grep -E "llm runtime selected|PONG|Error" | head -5
