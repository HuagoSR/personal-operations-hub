...
EOF
echo "profile ready (no models.json)"

node ~/tmp-proxy.js > /tmp/proxy6.log 2>&1 &
PROXY=$!
sleep 1
...
nohup bwrap --unshare-user --unshare-ipc --unshare-pid --unshare-uts --unshare-cgroup \
  ...
  --die-with-parent -- /opt/opencode-bin/opencode serve --port 4793 --hostname 127.0.0.1 > /tmp/oc6.log 2>&1 < /dev/null &
disown
for i in $(seq 1 30); do
  sleep 1
  curl -s -o /dev/null -w "%{http_code}" -m 5 http://127.0.0.1:4793/doc | grep -q 200 && break
done
echo "serve up"
node -e '
(async () => {
  const r1 = await fetch("http://127.0.0.1:4793/api/session", {
  ...
  console.log("sid:", sid);
  fetch("http://127.0.0.1:4793/global/event", { headers: { accept: "text/event-stream" } }).then(async (res) => {
    const reader = res.body.getReader(); const dec = new TextDecoder();
    while (true) { const { done, value } = await reader.read(); if (done) break; dec.decode(value, { stream: true }); }
  }).catch(() => {});
  await new Promise((r) => setTimeout(r, 2000));
  await fetch(`http://127.0.0.1:4793/api/session/${sid}/prompt`, {
    method: "POST", headers: { "Content-Type": "application/json" },
...
echo "=== proxy captured ==="
grep -o '"auth":"[^"]*"' /tmp/proxy6.log | head -3
grep -c chat/completions /tmp/proxy6.log
echo "=== serve log ==="
tail -n 5 /tmp/oc6.log
pkill -f "opencode serve --port 4793" 2>/dev/null
