...
j.experimental = { policies: [ { effect: "deny", action: "provider.use", resource: "*" }, { effect: "allow", action: "provider.use", resource: "deepseek" } ] };
j.provider.deepseek.options.baseURL = "http://127.0.0.1:8000/v1";
fs.writeFileSync(p, JSON.stringify(j, null, 2));
'
grep -A3 policies $H/.opencode/opencode.json | head -6

node ~/tmp-proxy.js > /tmp/proxy4.log 2>&1 &
PROXY=$!
sleep 1
...
nohup bwrap --unshare-user --unshare-ipc --unshare-pid --unshare-uts --unshare-cgroup \
  ...
  --die-with-parent -- /opt/opencode-bin/opencode serve --port 4791 --hostname 127.0.0.1 > /tmp/oc-proxy-serve4.log 2>&1 < /dev/null &
disown
for i in $(seq 1 30); do
  sleep 1
  curl -s -o /dev/null -w "%{http_code}" -m 5 http://127.0.0.1:4791/doc | grep -q 200 && break
done
echo "serve up"
node -e '
(async () => {
  const r1 = await fetch("http://127.0.0.1:4791/api/session", {
  ...
  fetch("http://127.0.0.1:4791/global/event", { headers: { accept: "text/event-stream" } }).then(async (res) => {
    const reader = res.body.getReader(); const dec = new TextDecoder();
    while (true) { const { done, value } = await reader.read(); if (done) break; dec.decode(value, { stream: true }); }
  }).catch(() => {});
  await new Promise((r) => setTimeout(r, 2000));
  await fetch(`http://127.0.0.1:4791/api/session/${sid}/prompt`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ prompt: { text: "PONG" } }),
  });
  console.log("prompt sent, waiting 40s");
...
echo "=== proxy captured ==="
grep -o '"auth":"[^"]*"' /tmp/proxy4.log | head -3
grep -c chat/completions /tmp/proxy4.log
pkill -f "opencode serve --port 4791" 2>/dev/null
node -e '
const fs = require("fs");
const p = process.env.HOME + "/wechat-linux-research/hub/data/workers/opencode/ex-19/home/.opencode/opencode.json";
const j = JSON.parse(fs.readFileSync(p, "utf8"));
j.provider.deepseek.options.baseURL = "https://api.deepseek.com/v1";
fs.writeFileSync(p, JSON.stringify(j, null, 2));
'
